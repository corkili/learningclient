...
public class ExamEditActivity extends AppCompatActivity 
    ...
    private Button addExamQuestionButton;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        saveButton = findViewById(R.id.exam_edit_button_save);
        deleteButton = findViewById(R.id.exam_edit_button_delete);
        cancelButton = findViewById(R.id.exam_edit_button_cancel);
        ...
        deleteButton.setOnClickListener(v -> {
            ...
            confirmDeleteDialog.setTitle("删除考试");
            confirmDeleteDialog.setMessage("确定删除该考试？");
    ...
    private void showQuestionScoreUpdateDialog(final QuestionInfo questionInfo) {
        AlertDialog.Builder scoreUpdateDialog =
                new AlertDialog.Builder(ExamEditActivity.this);
        final View dialogView = LayoutInflater.from(ExamEditActivity.this)
                .inflate(R.layout.dialog_score_editor,null);
        if (questionInfo.getQuestionType() != QuestionType.MultipleFilling) {
            dialogView.<TextView>findViewById(R.id.text_view_tip).setText(IUtils.format(
                    "本题是{}，请输入一个分数",
        ...
        scoreUpdateDialog.setView(dialogView);
        scoreUpdateDialog.setPositiveButton("确定",
                (dialog, which) -> {
                    EditText scoreEditView = dialogView.findViewById(R.id.text_edit_score);
    ...
    public boolean onItemLongClick(ViewHolder viewHolder) {
        ...
        confirmDeleteDialog.setPositiveButton("确定", (ed, which) -> {
            ...
            if (needRemoveIndex >= 0) {
                QuestionInfo questionInfo = questionInfos.remove(needRemoveIndex);
                questionScoreMap.remove(questionInfo.getQuestionId());
                recyclerViewAdapter.notifyDataSetChanged();
            }
        });
        confirmDeleteDialog.setNegativeButton("取消", (ed, which) -> {
